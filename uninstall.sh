...
if [ "$INSTALL_COPILOT" -eq 1 ]; then
  rm -f "${COPILOT_WEBSEARCH_DST}" "${COPILOT_WEBFETCH_DST}" "${COPILOT_HOOK_FILE}"
  printf 'Removed Copilot compatibility hooks if present:\n'
  printf '  - %s\n' "${COPILOT_WEBSEARCH_DST}"
  printf '  - %s\n' "${COPILOT_WEBFETCH_DST}"
  printf 'Removed Copilot user hook config if present:\n'
  printf '  - %s\n' "${COPILOT_HOOK_FILE}"
  printf 'Workspace Copilot hook file remains at:\n'
